...
class SWAGInference(object):
    ...
    def __init__(
        ...
        self._swag_diagonal_n = {name: 0 for name, param in self.network.named_parameters()}
        ...
        self.storage_columns={name: [] for name, param in self.network.named_parameters()}
    ...
    #        self._swag_diagonal_mean[name] = self._swag_diagonal_mean[name]+delta / self._swag_diagonal_n[name]
            self._swag_diagonal_mean[name]=(self._swag_diagonal_n[name]*self._swag_diagonal_mean[name]+param)/(self._swag_diagonal_n[name]+1)
           # self._swag_diagonal_var[name] += delta * (param - self._swag_diagonal_mean[name])
           # delta_2=param**2-self._swag_diagonal_n[name]*self._swag_diagonal_mean[name]**2-self._swag_diagonal_var[name]
    #        self._swag_diagonal_var[name]=self._swag_diagonal_var[name]+(param*param-self._swag_diagonal_var[name])/(self._swag_diagonal_n[name])
            self._swag_diagonal_var[name]=(self._swag_diagonal_n[name]*self._swag_diagonal_var[name]+param*param)/(self._swag_diagonal_n[name]+1)
        # Full SWAG
            if self.inference_mode == InferenceMode.SWAG_FULL:
                self._swag_diagonal_samples[name]=self._swag_diagonal_var[name]-(self._swag_diagonal_mean[name])**2
                self.storage_columns[name].append(self._swag_diagonal_samples[name])
            else:
                pass
        #print(self.storage_columns)
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        for _ in tqdm.trange(self.bma_samples, desc="Performing Bayesian model averaging"):
            ...
            for (xs,) in loader:
                ...
                model_sample_predictions.append(logits*0.3)
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            # Diagonal part
            sampled_param = current_mean + 0.13*current_std * z_1
            ...
            #    raise NotImplementedError("Sample parameter for full SWAG")
                D_hat=torch.stack(self.storage_columns[name][15:30], dim=-1)
             #   print(D_hat.shape)
              #  print('This is Dania {}'.format(D_hat))
                z_2=torch.randn([15])
                #print(z_2)
                sampled_param += 1/16* D_hat@z_2
    ...
    def predict_probabilities_map(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        for (batch_xs,) in loader:
            predictions.append(self.network(batch_xs)*0.3)
...
class SWAGScheduler(torch.optim.lr_scheduler.LRScheduler):
 ...
 #   def calculate_lr(self, current_epoch: float, old_lr: float,lr_start_factor=1e-3) -> float:
  #      return old_lr+old_lr*(lr_start_factor+(1-lr_start_factor)*0.5*(1+np.cos(current_epoch/30 *np.pi)))
   # def calculate_lr(self, current_epoch: float, old_lr: float,lr_start_factor=1e-3) -> float:
    #    return 1/(2*current_epoch+2)
